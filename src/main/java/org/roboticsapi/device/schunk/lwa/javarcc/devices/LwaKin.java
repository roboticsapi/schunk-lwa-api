...
public class LwaKin {
	...
	public LwaKin(double[] d, double[] theta, double[] a, double[] alpha, double[] min, double[] max) {
		this.d = d;
		this.theta = theta;
		this.a = a;
		this.alpha = alpha;
	...
	public List<double[]> invKin8(RPIFrame position) {
		...
		RPICalc.rpiToFrame(position, ourpos);
		...
		for (int iq1 = 0; iq1 < 2; ++iq1) {
			...
			List<double[]> q23 = calculate23(t15.getTranslation().getX(), -t15.getTranslation().getY(), Math.abs(a[1]),
					Math.abs(d[3]));
	...
	private List<double[]> calculate23(double x, double y, double a1, double a2) {
		double d = Math.sqrt(x * x + y * y);

		double v2 = Math.acos(((a1 * a1) + (a2 * a2) - (d * d)) / (2 * a1 * a2));
		...
		double delta = Math.atan2(x, y);

		double beta = Math.acos(((a1 * a1) + (d * d) - (a2 * a2)) / (2 * a1 * d));
	...
	private List<double[]> calculate456(MutableRotation r3e) {
